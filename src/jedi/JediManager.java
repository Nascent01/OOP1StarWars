...
public class JediManager {
    ...
    public static void demoteJedi(String[] args) throws Exception {
        ...
        if (!foundJedi.isPresent()) {
            ...
            if (foundJedi.get().getRank().equals(JediRank.YOUNGLING)) {
                ...
                Universe.getInstance().getJediPopulation().remove(jedi);
                ...
                Universe.getInstance().getJediPopulation().add(jedi);
            }
        }
    }
    ...
    public static void printJediInfo(String[] args) {
        ...
        for (Jedi jedi : jedis) {
            if (jedi.getName().equalsIgnoreCase(args[0])) {
                ...
                System.out.println("Inhibited planet: " + jedi.getPlanet().getName());
            }
        }
    }
    ...
    public static void getMostUsedSaberColorWithRank(String[] args) {
        ...
        Map<JediLightSaberColor, Long> saberJediRelation = jedis.stream()
                .filter(jedi -> jedi.getPlanet().getName().equalsIgnoreCase(planetName))
                .filter(jedi -> jedi.getRank().getRankName().equalsIgnoreCase(jediRank))
